...
| `mean_all` | 1 | 全部向量直接平均 |
| `manual_three` | 3 | 子文件夹分组取均值（模拟人工三分法） |
...
> - `manual_three`（子文件夹分组）与 `kmeans_k3` 性能接近，但 kmeans_k3 无需人工分文件夹，**可复现性更优**
...
**评估需要一个已切分好的数据集**。本项目提供 `scripts/prepare_lfw_dataset.py` 一键生成：
...
│             /subset_0/*.jpg          # manual_three 子文件夹（符号链接）
│             /subset_1/*.jpg
│             /subset_2/*.jpg
...
该脚本做的事：
1. 通过 `sklearn.datasets.fetch_lfw_people` 下载 LFW funneled 版本到 `~/scikit_learn_data/`
2. 筛选 ≥30 张/人的类别（约 34 人）
3. 按 `RANDOM_SEED=42` 固定打乱后 80/20 按人切分
4. 对 train 集每人建 3 个子文件夹（`subset_0/1/2`），图片均匀分配成符号链接

如果你想用自己的数据集（如同学照片），按同样目录结构组织即可。评估代码不关心图片从哪来——只读 `train/` 和 `test/` 两个子目录。
